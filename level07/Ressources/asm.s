...
main:
   ...
   0x0804872c <+9>:     sub    esp,0x1d0    ; alloc 464 bytes
   0x08048732 <+15>:    mov    eax,DWORD PTR [ebp+0xc]
   0x08048735 <+18>:    mov    DWORD PTR [esp+0x1c],eax  ; char **s_2 = argv 
   0x08048739 <+22>:    mov    eax,DWORD PTR [ebp+0x10]
   0x0804873c <+25>:    mov    DWORD PTR [esp+0x18],eax  ; char **s_3 = env
   0x08048740 <+29>:    mov    eax,gs:0x14
   0x08048746 <+35>:    mov    DWORD PTR [esp+0x1cc],eax ; int canary
   0x0804874d <+42>:    xor    eax,eax
   0x0804874f <+44>:    mov    DWORD PTR [esp+0x1b4],0x0 ; int i_6
   0x0804875a <+55>:    mov    DWORD PTR [esp+0x1b8],0x0 ; int i_5
   0x08048765 <+66>:    mov    DWORD PTR [esp+0x1bc],0x0 ; int i_4
   0x08048770 <+77>:    mov    DWORD PTR [esp+0x1c0],0x0 ; int i_3
   0x0804877b <+88>:    mov    DWORD PTR [esp+0x1c4],0x0 ; int i_2
   0x08048786 <+99>:    mov    DWORD PTR [esp+0x1c8],0x0 ; int i_1
   0x08048791 <+110>:   lea    ebx,[esp+0x24]   ; char_s1[406]
   0x08048795 <+114>:   mov    eax,0x0
   0x0804879a <+119>:   mov    edx,0x64
   0x0804879f <+124>:   mov    edi,ebx
   0x080487a1 <+126>:   mov    ecx,edx
   0x080487a3 <+128>:   rep stos DWORD PTR es:[edi],eax  ; memset(s_1, 0, 100)
   0x080487a5 <+130>:   jmp    0x80487ea <main+199>
   0x080487a7 <+132>:   mov    eax,DWORD PTR [esp+0x1c]  ; char **s_2
   0x080487ab <+136>:   mov    eax,DWORD PTR [eax] ; eax = argv[0]
   0x080487ad <+138>:   mov    DWORD PTR [esp+0x14],0xffffffff ; int i_7 = 0xffffffff
   0x080487b5 <+146>:   mov    edx,eax
   0x080487b7 <+148>:   mov    eax,0x0
   0x080487bc <+153>:   mov    ecx,DWORD PTR [esp+0x14]
   0x080487c0 <+157>:   mov    edi,edx
   0x080487c2 <+159>:   repnz scas al,BYTE PTR es:[edi]  ; strlen(argv[0])
   ...
   0x080487e0 <+189>:   call   0x80484f0 <memset@plt> ; memset(argv[0], 0, strlen(0))
...
store_number:
   ...
   0x08048633 <+3>:     sub    esp,0x28   ; 40 bytes
   0x08048636 <+6>:     mov    DWORD PTR [ebp-0x10],0x0  ; char_1[4]
   0x0804863d <+13>:    mov    DWORD PTR [ebp-0xc],0x0   ; char_2[4]
   0x08048644 <+20>:    mov    eax,0x8048ad3
   0x08048649 <+25>:    mov    DWORD PTR [esp],eax
   0x0804864c <+28>:    call   0x8048470 <printf@plt> ; printf(0x8048ad3)
   0x08048651 <+33>:    call   0x80485e7 <get_unum>
   0x08048656 <+38>:    mov    DWORD PTR [ebp-0x10],eax  ; char_1 = get_unum()
   0x08048659 <+41>:    mov    eax,0x8048add
   0x0804865e <+46>:    mov    DWORD PTR [esp],eax
   0x08048661 <+49>:    call   0x8048470 <printf@plt> ; printf(0x8048add)
   0x08048666 <+54>:    call   0x80485e7 <get_unum>
   0x0804866b <+59>:    mov    DWORD PTR [ebp-0xc],eax   ; ; char_2 = get_unum()
   ...
   0x08048688 <+88>:    je     0x8048697 <store_number+103> ; buf_2 % 3 == 0
   0x0804868a <+90>:    mov    eax,DWORD PTR [ebp-0x10]
   0x0804868d <+93>:    shr    eax,0x18
   0x08048690 <+96>:    cmp    eax,0xb7
   0x08048695 <+101>:   jne    0x80486c2 <store_number+146> ; buf_1 >> 24 != 183
   ...
   0x080486c5 <+149>:   shl    eax,0x2 ; buf_2 * 4
