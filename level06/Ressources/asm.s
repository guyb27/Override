...
main:
   ...
   0x0804887f <+6>:     sub    esp,0x50 ; alloc 80 bytes
   0x08048882 <+9>:     mov    eax,DWORD PTR [ebp+0xc]
   0x08048885 <+12>:    mov    DWORD PTR [esp+0x1c],eax ; buf_1[52]
   0x08048889 <+16>:    mov    eax,gs:0x14  ; canary protection
   0x0804888f <+22>:    mov    DWORD PTR [esp+0x4c],eax ; buf_2[4]
   ...
   0x080488e0 <+103>:   lea    eax,[esp+0x2c]   ; buf_2[32]
   0x080488e4 <+107>:   mov    DWORD PTR [esp],eax
   0x080488e7 <+110>:   call   0x8048550 <fgets@plt>    ; fgets(buf_2, 32, stdin)
   ...
   0x08048922 <+169>:   lea    edx,[esp+0x28]   ; buf_3[4]
   0x08048926 <+173>:   mov    DWORD PTR [esp+0x4],edx
   0x0804892a <+177>:   mov    DWORD PTR [esp],eax
   0x0804892d <+180>:   call   0x80485e0 <__isoc99_scanf@plt>   ; scanf("%u", buf_3, stdin)
   ...
   0x08048941 <+200>:   call   0x8048748 <auth> ; auth(buf_2, buf_3)
...
auth:
   ...
   0x0804878a <+66>:    jg     0x8048796 <auth+78>  ; jmp if strnlen > 5
   0x0804878c <+68>:    mov    eax,0x1
   0x08048791 <+73>:    jmp    0x8048877 <auth+303> ; return 1
   ...
   0x080487bd <+117>:   jne    0x80487ed <auth+165> ; jmp if ptrace != -1
   ...
   0x080487e8 <+160>:   jmp    0x8048877 <auth+303> ; return(1)
   ...
   0x080487f9 <+177>:   xor    eax,0x1337   ; buf_2[3] ^= 
   0x080487fe <+182>:   add    eax,0x5eeded ; buf_2[3] += 
   0x08048803 <+187>:   mov    DWORD PTR [ebp-0x10],eax ; buf_5[16]
   0x08048806 <+190>:   mov    DWORD PTR [ebp-0x14],0x0 ; int counter
   0x0804880d <+197>:   jmp    0x804885b <auth+275>
   0x0804880f <+199>:   mov    eax,DWORD PTR [ebp-0x14]
   0x08048812 <+202>:   add    eax,DWORD PTR [ebp+0x8]  ; buf_2[counter]
   0x08048815 <+205>:   movzx  eax,BYTE PTR [eax]
   0x08048818 <+208>:   cmp    al,0x1f
   0x0804881a <+210>:   jg     0x8048823 <auth+219> ; jmp if buf_2[counter] > 31
   0x0804881c <+212>:   mov    eax,0x1
   0x08048821 <+217>:   jmp    0x8048877 <auth+303> ; return(1)
   0x08048823 <+219>:   mov    eax,DWORD PTR [ebp-0x14]
   0x08048826 <+222>:   add    eax,DWORD PTR [ebp+0x8]
   0x08048829 <+225>:   movzx  eax,BYTE PTR [eax]   ; buf_2[counter]
   0x0804882c <+228>:   movsx  eax,al
   0x0804882f <+231>:   mov    ecx,eax
   0x08048831 <+233>:   xor    ecx,DWORD PTR [ebp-0x10] ; buf_2[i] ^= xored
   ...
   0x08048861 <+281>:   jl     0x804880f <auth+199> ; jmp if counter < strnlen
   0x08048863 <+283>:   mov    eax,DWORD PTR [ebp+0xc]
   0x08048866 <+286>:   cmp    eax,DWORD PTR [ebp-0x10]
   0x08048869 <+289>:   je     0x8048872 <auth+298>   ; jmp if 
